...
@Component({
  ...
  styleUrls: ['./trab-subopciones.component.css']
})
export class TrabSubopcionesComponent implements OnInit {
  ...
  public tblData(){
    ...
    this.tblSOpciones = {};
    this.rest.get('trabSubOpcion' , this.token, this.parametros).subscribe(data => {
        this.tblSOpciones = data;
    });
    setTimeout(()=> {
        this.carga = 'visible';
        this.loading = false;
  ...
  public Excel(){
    this.excel.exportAsExcelFile(this.tblSOpciones, 'sub-opciones');
     return false;
  }
